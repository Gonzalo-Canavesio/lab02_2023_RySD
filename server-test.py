...
class TestHFTPServer(TestBase):
    ...
    def test_connect_and_quit(self):
        ...
        self.assertEqual(
            w, [s], "Se envió quit, no hubo respuesta en %0.1f segundos" % TIMEOUT
        )
    ...
    def test_get_metadata(self):
        ...
        self.assertEqual(c.status, constants.CODE_OK)
        self.assertEqual(
            m, test_size, "El tamaño reportado para el archivo no es el correcto"
        )
        c.close()
    ...
    def test_get_full_slice(self):
        ...
        self.assertEqual(
            f.read(), test_data, "El contenido del archivo no es el correcto"
        )
        f.close()
        c.close()
    ...
    def test_partial_slices(self):
        ...
        f = open(self.output_file)
        self.assertEqual(
            f.read(), "a" * 100, "El contenido del archivo no es el correcto"
        )
        f.close()
        ...
        f = open(self.output_file)
        self.assertEqual(
            f.read(), "b" * 200, "El contenido del archivo no es el correcto"
        )
        f.close()
        ...
        f = open(self.output_file)
        self.assertEqual(
            f.read(), "c" * 100, "El contenido del archivo no es el correcto"
        )
        f.close()
        c.close()
...
class TestHFTPErrors(TestBase):
    ...
    def test_bad_argument_count(self):
        ...
        self.assertEqual(
            ...
            "El servidor no contestó 201 ante una lista de argumentos " "muy larga",
        )
        c.close()
    ...
    def test_bad_argument_count_2(self):
        ...
        self.assertEqual(
            status,
            constants.INVALID_ARGUMENTS,
            "El servidor no contestó 201 ante una lista de argumentos " "muy corta",
        )
        c.close()
...
class TestHFTPHard(TestBase):
    ...
    def test_multiple_commands(self):
        c = self.new_client()
        l = c.s.send("get_file_listing\r\nget_file_listing\r\n".encode("ascii"))
        assert l == len("get_file_listing\r\nget_file_listing\r\n".encode("ascii"))
        status, message = c.read_response_line(TIMEOUT)
        self.assertEqual(
            status,
            constants.CODE_OK,
            "El servidor no entendio muchos mensajes correctos " "enviados juntos",
        )
        c.connected = False
        c.close()
    ...
    def test_data_with_nulls(self):
        ...
        f = open(self.output_file)
        self.assertEqual(
            f.read(), test_data, "El contenido del archivo con NULs no es el correcto"
        )
        f.close()
        c.close()
    ...
    def test_long_file_listing(self):
        ...
        self.assertEqual(c.status, constants.CODE_OK)
        self.assertEqual(
            files, correct_list, "La lista de 1000 archivos no es la correcta"
        )
        c.close()
